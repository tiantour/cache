...
func (k *tKey) RENAMENX(key string) *redis.Resp {
	return operate("RENAMENX", key)
}

// RESTORE 反序列化给定的序列化值，并将它和给定的 key 关联。
func (k *tKey) RESTORE(key, value string, ttl int) *redis.Resp {
	return operate("RESTORE", key, ttl, value)
}

// SORT 返回或保存给定列表、集合、有序集合 key 中经过排序的元素。ASC/DESC ALPHA 字符串
func (k *tKey) SORT(key string, action ...interface{}) *redis.Resp {
	args := []interface{}{key}
...
func (k *tKey) TTL(key string) *redis.Resp {
	return operate("TTL", key)
}

// TYPE 返回 key 所储存的值的类型。
func (k *tKey) TYPE(key string) *redis.Resp {
	return operate("TYPE", key)
}

// SCAN 迭代当前数据库中的数据库键。
func (k *tKey) SCAN(key string, cursor int, pattern string, count int) *redis.Resp {
