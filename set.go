...
func (s Set) SADD(key string, value ...interface{}) *redis.Resp {
	args := []interface{}{key}
...
func (s Set) SDIFF(key ...interface{}) *redis.Resp {
	return operate("SDIFF", key...)
}

// SDIFFSTORE 这个命令的作用和 SDIFF 类似，但它将结果保存到 destination 集合，而不是简单地返回结果集。
func (s Set) SDIFFSTORE(dest string, key ...interface{}) *redis.Resp {
	args := []interface{}{}
...
func (s Set) SINTER(key ...interface{}) *redis.Resp {
	return operate("SINTER", key...)
}

// SINTERSTORE 这个命令类似于 SINTER 命令，但它将结果保存到 destination 集合，而不是简单地返回结果集。
func (s Set) SINTERSTORE(dest string, key ...interface{}) *redis.Resp {
	args := []interface{}{}
	args = append(args, key...)
...
func (s Set) SISMEMBER(key, member string) *redis.Resp {
...
func (s Set) SMOVE(key, dest, member string) *redis.Resp {
...
func (s Set) SRANDMEMBER(key string, count int) *redis.Resp {
	return operate("SRANDMEMBER", key, count)
}

// SREM 移除集合 key 中的一个或多个 member 元素，不存在的 member 元素会被忽略。
func (s Set) SREM(key ...interface{}) *redis.Resp {
	return operate("SREM", key...)
}

// SUNION 返回一个集合的全部成员，该集合是所有给定集合的并集。
func (s Set) SUNION(key ...interface{}) *redis.Resp {
	return operate("SUNION", key...)
}

// SUNIONSTORE 这个命令类似于 SUNION 命令，但它将结果保存到 destination 集合，而不是简单地返回结果集。
func (s Set) SUNIONSTORE(dest string, key ...interface{}) *redis.Resp {
	args := []interface{}{dest}
	args = append(args, key...)
...
// SSCAN scan
func (s Set) SSCAN(key string, cursor int, pattern string, count int) *redis.Resp {
